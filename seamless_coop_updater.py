...
reg = winreg.ConnectRegistry(None, win32con.HKEY_LOCAL_MACHINE)

try:
    print("[!] Opening registry key...")
    key = winreg.OpenKey(win32con.HKEY_LOCAL_MACHINE,
...
try:
    ...
    print("[x] An error occurred while trying to parse"
...
if not data["libraryfolders"]:
    print("[x] An error occurred while trying to use data"
          "from your libraryfolders.vdf file")
    sys.exit(-1)
else:
    for key in data["libraryfolders"]:
        if key.isdigit():
            games_path.append(data["libraryfolders"]
                              [key]['path'] + "\\steamapps")
            ...
            if app_id in file:
                if os.path.splitext(file)[1] == ".acf":
                    try:
                        ...
                        print(f"[x] A problem occurred while trying to open"
                              f" {os.path.join(path, file)}."
                              f"Make sure you gave the permission"
                              f" to read this path")
                        sys.exit(-1)
                    except TypeError or ValueError:
                        print(f"[x] An error occurred while trying to parse \
                    ...
                    if data is not None and \
                            data["AppState"]["installdir"] is not None:
                        directory = data["AppState"]["installdir"]
                        elden_ring_path = path + "\\common\\" \
                            + directory + "\\Game"
                    else:
...
git_repo = 'https://api.github.com/repos/'\
    'LukeYui/EldenRingSeamlessCoopRelease/releases/latest'
...
try:
    ...
    archive.extract('SeamlessCoop/elden_ring_seamless_coop.dll',
                    elden_ring_path)
    archive.close()
except RuntimeError as e:
    print(f"Unexpected {e}=, {type(e)}")
